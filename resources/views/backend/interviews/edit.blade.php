        ...
        <div class="page-content">
                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Edit Interview</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body p-4">
                    <h5 class="mb-4">Edit Interview</h5>
                    <form action="{{route('admin.interviews.update',[Crypt::encrypt($data->id)])}}" id="jQueryValidationForm" method="POST" enctype="multipart/form-data">@csrf
                        @method('put')
                        <div class="row g-3">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                ...
                                <select name="candidate_id" id="candidate_id" class="form-select mb-3" required>
                                    <option value="{{$data->candidate_id}}">{{$candidate->candidate_name}}</option>
                                </select>
                                @error('candidate_id')
                                    <span class="error mt-2 text-danger" role="alert">
                                        <strong>{{ $message }}</strong>
...
@section('javascripts')
    ...
    <script>
        ...
        function prefillData(){
            $('#interview_name').val('{{$data->interview_name}}');
            $('#candidate_id').val('{{$data->candidate_id}}');
            $('#client_id').val('{{$data->client_id}}');
            $('#job_opportunity_id').val('{{$data->job_opportunity_id}}');
            $('#from_date').val('{{$data->from_date}}');
            $('#to_date').val('{{$data->to_date}}');
            $('#interview_owner_id').val('{{$data->interview_owner_id}}');
